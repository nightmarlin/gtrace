...
let commands: Command[] = [
  ...
  new Command([`ping`], `responds with 'pong'`, `ping`, function (
    params,
    msg,
    cmd,
    config
...
async function messageHandler(msg: djs.Message) {
  ...
  if (msg?.mentions?.members?.has(msg.client.user.id)) {
    let authorised = msg.guild.members
      .resolve(msg.author.id)
      .roles.cache.has(config.controlRole);
    msg
      .reply(
        `hi there! my prefix is \`${config.prefix}\`${
          authorised
            ? ``
            : ` - but you can't use me as you dont have my management role!`
        }`
      )
      .catch((err) =>
        console.log(`unable to send prefix info message due to: ${err}`)
      );
    return;
  }
  ...
  if (!author.roles.cache.has(config.controlRole)) {
    msg
      .reply(`you don't have permission to use this bot`)
      .catch((err) =>
        console.error(
          `unable to send permission failure message due to: ${err}`
        )
      );
    return;
  }
  ...
  if (res === undefined) {
    ...
    msg
      .reply(`**${params[0]}** is not a command!`)
...
function main() {
  ...
  client.once("ready", () => {
    ...
    console.log(
      `invite me using 'https://discordapp.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=268504064'`
    );

...
async function runRemoval(g: djs.GuildMember, refDate: Date, config: Config) {
  let dmc = await g.createDM().catch((err) => {
    console.error(
      `unable to create dm channel with ${g.user.username}#${g.user.discriminator} (${g.id}) due to: ${err}`
    );
  ...
  if (dmc !== undefined) {
    dmc
      .send(generateText(g, true, refDate, config), {
        allowedMentions: { parse: [] },
      })
      .catch((err) =>
        console.error(`unable to send role removal dm due to: ${err}`)
      );
  }
...
async function runAlert(g: djs.GuildMember, refDate: Date, config: Config) {
  ...
  if (dmc !== undefined) {
    dmc
      .send(generateText(g, false, refDate, config), {
        allowedMentions: { parse: [] },
      })
      .catch((err) =>
        console.error(`unable to send role removal dm due to: ${err}`)
...
function generateText(
  g: djs.GuildMember,
  removeOrAlert: Boolean,
  refDate: Date,
  config: Config
): string {
  let res = `hello ${g.user.username}!
your last greet in **${g.guild.name}** was ${
    removeOrAlert ? `before` : `just after`
...
export function runFollowUps(fui: FollowUpInfo, config: Config) {
  ...
  for (const g of fui.toAlert) {
    console.log(
      `alerting ${g.user.username}#${g.user.discriminator} (${g.id})`
    );
    runAlert(g, fui.refDate, config);
  }
  for (const g of fui.toRemove) {
    console.log(
      `removing ${g.user.username}#${g.user.discriminator} (${g.id})`
    );
  ...
  fui.replyTo
    .reply(`follow-ups complete!`)
    .catch((err) =>
      console.log(`unable to send follow up completion message due to: ${err}`)
    );
}
